...
def test_canonical_version_is_valid_and_read_by_build_script():
    ...
    assert (REPO_ROOT / "VERSION").read_text(encoding="ascii").strip() == "1.7.1"
    assert build.read_app_version() == ("1.7.1", (1, 7, 1))
...
def test_windows_version_resource_is_generated_from_canonical_version(tmp_path, monkeypatch):
    ...
    build.write_version_info("1.7.1", (1, 7, 1))

    text = generated.read_text(encoding="utf-8")
    assert "filevers=(1, 7, 1, 0)" in text
    assert "prodvers=(1, 7, 1, 0)" in text
    assert "StringStruct('ProductVersion', '1.7.1.0')" in text
...
def test_package_build_receives_canonical_version(tmp_path, monkeypatch):
    ...
    assert build.run_native_package_build("linux", "linux", script, "AppImage", "1.7.1") == 0
    assert called == [["bash", str(script), "1.7.1"]]
