...
- **Clipboard Exposure**: The clipboard auto-clear feature attempts to erase the active clipboard after a user-defined delay only if its content is unchanged. Native Windows copies request exclusion from built-in Clipboard History and cloud clipboard sync, but Paracci cannot guarantee admission is prevented or delete retained history entries. Browser-only copies and third-party clipboard managers remain outside that protection.
- **Secure File Deletion**: Paracci's secure-delete hygiene overwrites file content in place, attempts platform-specific flush or deallocation hints, and unlinks the file. Journaling or copy-on-write filesystems, SSD wear-leveling algorithms, system snapshots, cloud-sync daemons, and system backups may still retain copies on physical storage. For encrypted message content, encryption key protection or destruction remains the stronger security boundary.
...
| --- | --- | --- | --- |
| **Windows** | Attempts `SetWindowDisplayAffinity` to hide the window from capture software. Does not block physical cameras, remote desktop software, or administrator-level capture tools. | In-place overwrite, `fsync`, and best-effort `FSCTL_FILE_LEVEL_TRIM` hint before unlink; physical erasure is not guaranteed. | Native copies clear the unchanged active clipboard after the delay and request `ExcludeClipboardContentFromMonitorProcessing`. Paracci does not delete clipboard-history entries; built-in or third-party history may retain copies. |
| **macOS** | Attempts `NSWindowSharingNone` to restrict window sharing. Does not block native screenshots, screen recordings, or hardware capture. | In-place overwrite and best-effort `F_FULLFSYNC` (`fsync` fallback) before unlink; physical erasure is not guaranteed. | Auto-clear after delay; Paracci implements no system clipboard-history control on macOS. |
| **Linux** | Unimplemented. Screenshot blocking is compositor-specific and not supported across X11 and Wayland environments. | In-place overwrite, `fdatasync`, and optional per-file `FALLOC_FL_PUNCH_HOLE` deallocation before unlink; physical erasure is not guaranteed. | Auto-clear using `xclip` or `wl-copy` if installed; Paracci implements no system clipboard-history control on Linux. |

When the UI is opened in browser-only mode, it cannot add the Windows native history-exclusion marker. Paracci warns users that clipboard history may retain the copied text and may require manual clearing.
